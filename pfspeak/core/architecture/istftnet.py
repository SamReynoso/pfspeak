...
class AdaINResBlock1(nn.Module):
    ...
    def forward(self, x, s):
        data = zip(self.convs1, self.convs2, self.adain1,
                   self.adain2, self.alpha1, self.alpha2)
        for c1, c2, n1, n2, a1, a2 in data:
            xt = n1(x, s)
...
class TorchSTFT(nn.Module):
    ...
    def transform(self, input_data):
        forward_transform = torch.stft(input_data,
                                       self.filter_length,
                                       self.hop_length,
                                       self.win_length,
    ...
    def inverse(self, magnitude, phase):
        inverse_transform = torch.istft(magnitude * torch.exp(phase * 1j),
                                        self.filter_length,
                                        self.hop_length,
                                        self.win_length,
...
class SineGen(nn.Module):
    ...
    def _f02sine(self, f0_values):
        ...
        rand_ini = torch.rand(f0_values.shape[0],
                              f0_values.shape[2],
        ...
        if not self.flag_for_pulse:
            ...
            phase = (
                    F.interpolate(phase.transpose(1, 2) * self.upsample_scale,
...
class Generator(nn.Module):
    def __init__(self,
        ...
        self.m_source = (
                SourceModuleHnNSF(
                    sampling_rate=24000,
                    upsample_scale=(
                        math.prod(upsample_rates) * gen_istft_hop_size
                        )
                    ,
                    harmonic_num=8,
                    voiced_threshod=10)
        ...
        for i in range(len(self.ups)):
            ...
            for _, (k, d) in enumerate(
                    zip(resblock_kernel_sizes,
                        resblock_dilation_sizes
                        )
                    ):
                self.resblocks.append(AdaINResBlock1(ch,
                                                     k,
                                                     d,
                                                     style_dim
                                                     )
    ...
    def forward(self, x, s, f0):
        with torch.no_grad():
            ...
            # NOTE: noi_source and uv ar unused
...
class Decoder(nn.Module):
    def __init__(self, dim_in, style_dim, dim_out, 
        ...
        self.decode.append(AdainResBlk1d(1024 + 2 + 64, 1024, style_dim))
        self.decode.append(AdainResBlk1d(1024 + 2 + 64, 1024, style_dim))
        self.decode.append(AdainResBlk1d(1024 + 2 + 64,
                                         512,
                                         style_dim,
                                         upsample='not none',
                                         )
