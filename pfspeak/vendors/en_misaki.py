# Based on misaki.en from commit TODO: Add commit hash.
# Modifications:
# - lazy spaCy loading
# - removed NumPy dependency
# - lazy lexicon loading
...
class Lexicon:
    ...
    def get_NNP(self, word):
        # pf
        ps = [self.golds[c.upper()] for c in word if c.isalpha()]
        if None in ps:
            return None, None
...
class G2P:
    ...
    def lexicon(self):
        if self._lexicon is None:
            self._lexicon = Lexicon(self.british)
    ...
    def nlp(self):
        if self._nlp is None:
            ...
            name = f"en_core_web_{'trf' if self.__trf else 'sm'}"
    ...
    def tokenize(self, text: str, tokens, features) -> List[MToken]:
        ...
        for k, v in features.items():
            ...
            head = True
            for j, value in enumerate(align.y2x.data):
                if value != k:
                    continue

                token = mutable_tokens[j]
                if not isinstance(v, str):
                    token.stress = v
                elif v.startswith('/'):
                    token.is_head = head
                    token.phonemes = v[1:] if head == 0 else ""
                    token.rating = 5
                    head = False
                # elif v.startswith('['):
